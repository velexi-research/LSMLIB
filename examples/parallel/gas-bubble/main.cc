...
int main(int argc, char *argv[])
{
    ...
    // Set PatchData handles
    velocity_field_module->setDataHandles(phi_handle, control_volume_handle);
    ...
    while ( !lsm_algorithm->endTimeReached() &&
        ...
        double implied_radius = -1;
        if (dim.getValue() == 3) {
            tbox::pout << "  Volume:  " << volume << endl;
        
            implied_radius = pow(0.75 * volume / M_PI, 1.0/3.0);

        } else if (dim.getValue() == 2) {
            tbox::pout << "  Area:  " << volume << endl;

            implied_radius = sqrt(volume / M_PI);
        }
        tbox::pout << "    (implied radius):  " << implied_radius << endl;
        ...
        if (count == 0) {
            ...
            if (dim.getValue() == 3) {
                tbox::pout << "  Surface area:  "
                           << surface_area << endl;
                implied_radius = sqrt(0.25 * surface_area / M_PI);

            } else if (dim.getValue() == 2) {
                tbox::pout << "  Perimeter:  "
                           << surface_area << endl;

                implied_radius = 0.5 * surface_area / M_PI;
            }
        }
        tbox::pout << "    (implied radius):  " << implied_radius << endl;
