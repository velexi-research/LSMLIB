...
int main(int argc, char *argv[])
{
  ...
  Pointer< CellVariable<3,double> > phi_variable =
    new CellVariable<3,double>("phi_test",1);
  Pointer< CellVariable<3,double> > grad_phi_variable =
    new CellVariable<3,double>("grad(phi)",3);
...
*/
   ...
   * of the velocity.  For the test velocity field 0, this integral
   * is the volume of the sphere bounded by the zero level set.
   */
  double volume_integral = LevelSetMethodToolbox<3>::computeVolumeIntegral(
    patch_hierarchy,
    velocity_handle, 
    phi_handle,
   ...
   */
  double surface_integral = LevelSetMethodToolbox<3>::computeSurfaceIntegral(
    patch_hierarchy,
    velocity_handle, 
    phi_handle,
    grad_phi_plus_handle,
