...
extern "C" {
  #include "testlsm_3d_velocityfield_fort.h"
}
...
/* Constructor */
TestLSM_3d_VelocityFieldModule::TestLSM_3d_VelocityFieldModule(
  ...
  const string& object_name)
{
  ...
  Pointer< CellVariable<3,double> > velocity = 
    new CellVariable<3,double>("velocity field",3); 
...
void TestLSM_3d_VelocityFieldModule::computeVelocityField(
  const double time,
...
/* initializeLevelData() */
void TestLSM_3d_VelocityFieldModule::initializeLevelData (
...
void TestLSM_3d_VelocityFieldModule::computeVelocityFieldOnLevel(
  ...
  const double time) 
{
  for (PatchLevelIterator<3> pi(level); pi; pi++) { // loop over patches
    ...
    double* vel_z_data_ptr = velocity_data->getPointer(2);
...
void TestLSM_3d_VelocityFieldModule::printClassData(ostream& os) const
{
  os << "\nTestLSM_3d_VelocityFieldModule::printClassData..." << endl;
  os << "TestLSM_3d_VelocityFieldModule: this = " << 
     (TestLSM_3d_VelocityFieldModule*)this << endl;
...
void TestLSM_3d_VelocityFieldModule::getFromInput(
  Pointer<Database> db)
{
#ifdef DEBUG_CHECK_ASSERTIONS
  assert(!db.isNull());
