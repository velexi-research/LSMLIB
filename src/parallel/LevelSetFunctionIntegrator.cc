...
/* computeStableDt() */
template <int DIM> 
double LevelSetFunctionIntegrator<DIM>::computeStableDt()
{
  /*
   * compute maximum stable dt using:
   *
   *   if (user_specified_dt < DBL_MAX)
   *
  ...
  double max_user_specified_dt = DBL_MAX;
  ...
  for ( int ln=0 ; ln < num_levels; ln++ ) {
    ...
    for (pi.initialize(level); pi; pi++) { // loop over patches
       ...
       */
      double user_specified_dt_on_patch = 
  ...
  double max_stable_dt = -1; // temporary value to be set below
  if (max_user_specified_dt < DBL_MAX) {
    ...
    for (int comp = 0; comp < d_num_level_set_fcn_components; comp++) {
      ...
      if (d_lsm_velocity_field_strategy->providesExternalVelocityField()) {
        double max_advection_dt_for_component = 
      ...
      if (d_lsm_velocity_field_strategy->providesNormalVelocityField()) {
        ...
        double max_phi_normal_vel_dt_for_component = 
        ...
        if (d_codimension == 2) {
          ...
          double max_psi_normal_vel_dt_for_component = 
            LevelSetMethodToolbox<DIM>::computeStableNormalVelocityDt(
              d_patch_hierarchy,
              d_lsm_velocity_field_strategy->
    ...
    double physics_dt = d_lsm_velocity_field_strategy->computeStableDt();
  ...
  const bool uses_richardson_extrapolation_too)
{
  ...
  for (pi.initialize(level); pi; pi++) { // loop over patches
    ...
    Pointer< CellData<DIM,double> > psi_data;
    if (d_codimension == 2) {
  ...
  const int component)
{
  ...
  for ( int ln=0 ; ln < num_levels; ln++ ) {
    ...
    for (pi.initialize(level); pi; pi++) { // loop over patches
      ...
      Pointer< CellData<DIM,double> > rhs_data =
        patch->getPatchData( rhs_handle );
      Pointer< CellData<DIM,double> > grad_phi_upwind_data =
        patch->getPatchData( grad_phi_upwind_handle );
      Pointer< CellData<DIM,double> > velocity_data =
      ...
      double* rhs = rhs_data->getPointer();
      double* grad_phi_upwind[LSM_DIM_MAX];
      double* vel[LSM_DIM_MAX];
      for (int dim = 0; dim < DIM; dim++) {
  ...
  const int component)
{
  ...
  for ( int ln=0 ; ln < num_levels; ln++ ) {
    ...
    for (pi.initialize(level); pi; pi++) { // loop over patches
      ...
      Pointer< CellData<DIM,double> > rhs_data =
        patch->getPatchData( rhs_handle );
      Pointer< CellData<DIM,double> > normal_velocity_data =
      ...
      double* vel = normal_velocity_data->getPointer();
      for (int dim = 0; dim < DIM; dim++) {
...
void LevelSetFunctionIntegrator<DIM>::initializeVariables()
{
  ...
  if (var_db->checkVariableExists("phi (LSMLIB)")) {
    ...
    phi_variable = new CellVariable<DIM,double>(
  ...
  if (var_db->checkVariableExists("grad phi (LSMLIB)")) {
    ...
    grad_phi_variable = new CellVariable<DIM,double>("grad phi (LSMLIB)",DIM); 
  }
  ...
  // RHS for phi updates
  Pointer< CellVariable<DIM,double> > rhs_phi_variable;
  if (var_db->checkVariableExists("rhs phi (LSMLIB)")) {
    rhs_phi_variable = var_db->getVariable("rhs phi (LSMLIB)");
  } else {
    rhs_phi_variable = new CellVariable<DIM,double>("rhs phi (LSMLIB)",1); 
  }
  ...
  if (d_codimension == 2) {
    ...
    // RHS for psi updates
    Pointer< CellVariable<DIM,double> > rhs_psi_variable;
    if (var_db->checkVariableExists("rhs psi (LSMLIB)")) {
      rhs_psi_variable = var_db->getVariable("rhs psi (LSMLIB)");
    } else {
      rhs_psi_variable = new CellVariable<DIM,double>("rhs psi (LSMLIB)",1); 
    }
   ...
   */
  Pointer< CellVariable<DIM,double> > control_volume;
  if (var_db->checkVariableExists("control volume (LSMLIB)")) {
    control_volume = var_db->getVariable("control volume (LSMLIB)");
  } else {
    control_volume = new CellVariable<DIM,double>("control volume (LSMLIB)",1);
  }
