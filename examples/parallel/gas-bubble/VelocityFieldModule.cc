...
LSMLIB_REAL VelocityFieldModule::computeStableDt()
{
    ...
    // Estimate stable dt using formulas for V_n = a - b * kappa when
    // `a` and `b` are constants.

    // compute current volume of bubble
    ...
    double pressure_difference = d_K * M_PI / bubble_volume - d_P_ext;
    ...
    const_normal_velocity_term *= abs(pressure_difference);
  ...
  const int component)
{
  ...
  for ( int ln=0 ; ln<=finest_level ; ln++ ) {
    ...
    computeVelocityFieldOnLevel(level, time, bubble_volume, phi_handle);
    ...
    const int phi_handle)
{
    ...
    for (hier::PatchLevel::Iterator pi(level->begin()); pi!=level->end();
        ...
        if (num_dims == 3) {
            ...
            // compute normal velocity = (K*PI/V - P_ext) - sigma * kappa
            d_math_ops.scale(velocity_data, -d_sigma, kappa_data, fillbox);

            double pressure_difference = d_K * M_PI / bubble_volume - d_P_ext;
            d_math_ops.addScalar(velocity_data, velocity_data,
                                 pressure_difference, fillbox);
    ...
    boost::shared_ptr<tbox::Database> db)
{
    ...
    d_K = db->getDouble("K");
    d_P_ext = db->getDoubleWithDefault("P_ext", 0);
    d_sigma = db->getDouble("sigma");
