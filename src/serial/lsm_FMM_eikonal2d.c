...
/* 
 * FMM_updateGridPoint_Eikonal2d_Order2() computes and returns the 
 * updated phi value of the specified grid point using values of 
 ...
 */
double FMM_updateGridPoint_Eikonal2d_Order2(
  FMM_CoreData *fmm_core_data,
  FMM_FieldData *fmm_field_data,
  int *grid_idx,
  int num_dims,
  ...
  for (dir = 0; dir < LSM_FMM_2D_NDIM; dir++) { /* loop over coord directions */
    ...
    if (!LSM_FMM_2D_IDX_OUT_OF_BOUNDS(neighbor[0],neighbor[1],grid_dims)) {
      ...
      if (KNOWN == neighbor_status) {
         ...
         */
        if (LSM_FMM_2D_ABS(phi_plus) < LSM_FMM_2D_ABS(phi_upwind)) {
          phi_upwind = phi_plus;
        }
      }
    }
  ...
  double *dx)
{
  ...
  double inv_dx_sq; 
  int offset[LSM_FMM_2D_NDIM]; 
  int neighbor1[LSM_FMM_2D_NDIM];
  int neighbor2[LSM_FMM_2D_NDIM];
  ...
  for (dir = 0; dir < LSM_FMM_2D_NDIM; dir++) { /* loop over coord directions */
    ...
    if (!LSM_FMM_2D_IDX_OUT_OF_BOUNDS(neighbor1[0],neighbor1[1],grid_dims)) {
      idx_neighbor1 = LSM_FMM_2D_IDX(neighbor1[0],neighbor1[1]);
      ...
      if (KNOWN == neighbor_status) {
        ...
        if (!LSM_FMM_2D_IDX_OUT_OF_BOUNDS(neighbor2[0],neighbor2[1],
                                          grid_dims)) {
          idx_neighbor2 = LSM_FMM_2D_IDX(neighbor2[0],neighbor2[1]);
          neighbor_status = (PointStatus) gridpoint_status[idx_neighbor2];
          if ( (KNOWN == neighbor_status) &&
               (LSM_FMM_2D_ABS(phi_upwind2) < LSM_FMM_2D_ABS(phi_upwind1)) ) {
    ...
    if (!LSM_FMM_2D_IDX_OUT_OF_BOUNDS(neighbor1[0],neighbor1[1],grid_dims)) {
      ...
      if (KNOWN == neighbor_status) {
         ...
         *       we define our own ABS macro
         */
        if (LSM_FMM_2D_ABS(phi_plus) < LSM_FMM_2D_ABS(phi_upwind1)) {
